...
//| ile birebir ayni -- bu dosya SADECE cok-sembol orkestrasyonunu    |
//| ekliyor: her sembol icin ayri ATR/EMA handle + bar takibi, ve     |
//| aninda islem risklerinin toplamini sinirlayan portfoy risk tavani.|
//| TradeBot_NOA.mq5'e (tek sembol GOLD) DOKUNULMADI -- o ayri, kanit-|
//| lanmis haliyle kalmaya devam ediyor.                               |
//|                                                                    |
//| Kullanim: Herhangi bir grafige (sembolu onemli degil, EA sadece   |
//| zamanlayiciyla calisiyor) eklenir, AutoTrading acik olmali.        |
//+------------------------------------------------------------------+
...
//--- Genel ayarlar
input string  TradedSymbolsCsv          = "GOLD,GBPJPY,GBPUSD,GBPAUD,EURAUD,BTCUSD,SILVER,EURJPY,EURNZD,USDCHF";
input ENUM_TIMEFRAMES TF                = PERIOD_M30;
input double  RiskPercentPerTrade       = 1.0;      // islem basi risk (% bakiye)
input double  MaxPortfolioRiskPercent   = 4.0;      // ayni anda acik EA pozisyonlarinin toplam risk tavani (% bakiye)
input double  MaxLotCap                 = 5.0;      // maksimum lot siniri (guvenlik kepi, sembol basina)
input int     MagicNumber               = 20260826;
input int     PollSecondsTimer          = 30;       // OnTimer periyodu (saniye) -- grafigin kendi tick'ine bagli degil

//--- FVG parametreleri (Python: strategy/fvg.py ile birebir, tum semboller icin ortak)
...
//--- Dahili durumlar (sembol basina, index TradedSymbols[] ile hizali)
string   symbols[];
int      atrHandle[];
int      emaHandle[];
...
//+------------------------------------------------------------------+
//| Yardimci yapilar (Python'daki dataclass'lara karsilik)             |
//+------------------------------------------------------------------+
struct Candle
  {
   datetime time;
...
//+------------------------------------------------------------------+
//| CSV sembol listesini parse edip her biri icin ATR/EMA handle acar |
//+------------------------------------------------------------------+
int OnInit()
  {
   ...
   for(int i = 0; i < n; i++)
     {
      ...
      atrHandle[i] = iATR(symbols[i], TF, ATR_Period);
      emaHandle[i] = iMA(symbols[i], TF, EMA_Period, 0, MODE_EMA, PRICE_CLOSE);
      lastBarTime[i] = 0;

      if(atrHandle[i] == INVALID_HANDLE || emaHandle[i] == INVALID_HANDLE)
        {
         Print("[INDICATOR_INIT_FAILED] Indikator handle olusturulamadi: ", symbols[i]);
         return(INIT_FAILED);
        }
     }

   Print("[INIT_OK] ", n, " sembol icin coklu-sembol EA baslatildi: ", TradedSymbolsCsv);
...
//+------------------------------------------------------------------+
//| OnTimer - her sembol icin yeni bar acilip acilmadigini kontrol eder|
//+------------------------------------------------------------------+
void OnTimer()
  {
   for(int i = 0; i < ArraySize(symbols); i++)
     {
      ...
      if(curBarTime == 0 || curBarTime == lastBarTime[i])
         continue;  // veri yok ya da ayni mum icinde tekrar calismasin
      lastBarTime[i] = curBarTime;

      EvaluateAndTrade(symbols[i], atrHandle[i], emaHandle[i]);
     }
  }

// OnTick de birakiliyor -- EA hangi grafige eklendiyse o sembolun tick'lerinde de
// bar kapanisi kontrolu tetiklenir, boylece OnTimer'a ek bir guvence katmani olur.
void OnTick()
  {
   OnTimer();
  }
...
//+------------------------------------------------------------------+
//| Ana degerlendirme: tek bir sembol icin veriyi cek, sinyal ara      |
//+------------------------------------------------------------------+
void EvaluateAndTrade(const string symbol, int hAtr, int hEma)
  {
   int lookback = 500;
   ...
   if(CopyRates(symbol, TF, 1, bars, rates) != bars)
     {
      Print("[BUFFER_ALIGNMENT_FAILED] CopyRates failed or incomplete for ", symbol);
      return;
     }

   double atr[];
   ArraySetAsSeries(atr, false);
   if(CopyBuffer(hAtr, 0, 1, bars, atr) != bars)
     {
      Print("[BUFFER_ALIGNMENT_FAILED] CopyBuffer ATR failed or incomplete for ", symbol);
      return;
     }

   double ema[];
   ArraySetAsSeries(ema, false);
   if(CopyBuffer(hEma, 0, 1, bars, ema) != bars)
     {
      Print("[BUFFER_ALIGNMENT_FAILED] CopyBuffer EMA failed or incomplete for ", symbol);
      return;
     }
   ...
   for(int i = 0; i < bars; i++)
     {
      ...
      if(!MathIsValidNumber(atr[i]) || atr[i] <= 0)
        {
         Print("[BUFFER_ALIGNMENT_FAILED] Invalid ATR value at index ", i, " for ", symbol);
         return;
        }
      if(!MathIsValidNumber(ema[i]))
        {
         Print("[BUFFER_ALIGNMENT_FAILED] Invalid EMA value at index ", i, " for ", symbol);
         return;
        }
     }
      ...
      for(int j = 0; j < ArraySize(obs); j++)
        {
         ...
         TryOpenTrade(symbol, wantsUp, entry, sl, tp, "A+");
         return;
        }
     }
  }
   ...
   for(int i = 0; i < ArraySize(levels); i++)
     {
      ...
      if(levels[i].lastIdx > signalIdx) continue;
...
//+------------------------------------------------------------------+
//| Sembole gore acik pozisyon kontrolu                               |
//+------------------------------------------------------------------+
         ...
         if(PositionGetString(POSITION_SYMBOL) == symbol)
           {
            ...
            if(posMagic == MagicNumber)
              {
               Print("[EXISTING_EA_POSITION] Trade skipped: Open position exists for ", symbol, " with MagicNumber ", MagicNumber);
               return true;
              }
            else if(!isHedging)
              {
...
//+------------------------------------------------------------------+
//| Portfoy risk tavani: su an acik olan TUM EA pozisyonlarinin        |
//| toplam riskini (% bakiye, SL mesafesine gore) hesaplar.            |
//+------------------------------------------------------------------+
   ...
   for(int i = 0; i < total; i++)
     {
      ...
      if(slPrice <= 0)
         continue;  // SL yoksa risk hesaplanamaz, atla (guvenlik icin sifir sayilmaz, sadece dahil edilmez)
...
void TryOpenTrade(const string symbol, bool isBuy, double entry, double sl, double tp, string reason)
  {
   ...
   //--- Portfoy risk tavani: bu islem eklendiginde toplam risk asilacak mi?
