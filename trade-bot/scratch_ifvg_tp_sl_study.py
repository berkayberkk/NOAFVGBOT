...
# iFVG'nin kendi R'sinde (asagida) en cok SL yedigi zaman dilimi
# (strategy/config.py:MODULE_DISABLED_TIMEFRAMES) artik TARANMIYOR --
# 2026-08-31 karariyla iFVG bu zaman diliminde islem acmiyor.
TIMEFRAMES = [tf for tf in (Timeframe.M30, Timeframe.H1, Timeframe.H2, Timeframe.H4, Timeframe.D1, Timeframe.W1)
              if tf.name not in MODULE_DISABLED_TIMEFRAMES["ifvg"]]
R_MULTIPLES = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0]
IFVG_OFFICIAL_R_MULTIPLE = MODULE_R_MULTIPLE["ifvg"]  # = 1.5 -- beklenti (expectancy_r) tepe noktasi, resmi TP hedefi
SL_BUFFER_RATIO = 1.0
MAX_WAIT_BARS = 3000
