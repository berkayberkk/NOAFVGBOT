"""
PROTOTIP (scratch, kalici degil): scratch_sl_buffer_recalibration_study.py'nin
DEVAMI. Ilk tarama (0.25-3.0 FVG, 0.5-5.0 OB) hem win rate hem expectancy'nin
R-katı taramasinin aksine BIRLIKTE ve MONOTON iyilestigini, ustelik taranan
araligin EN UST UCUNDA hala iyilesmeye devam ettigini (plato/donus noktasi
gorulmedi) buldu. Bu script cok daha genis bir SL tamponu araligini tarayarak
gercek optimum/donus noktasini (varsa) bulmaya calisir -- ya da egilimin
gercekten sinirsiz devam ettigini (yani "stop'u ne kadar genis tutarsan o
kadar iyi" mekanizmasinin sonuna kadar dogru oldugunu) dogrular.
"""
...
SL_RATIOS_FVG = [3.0, 5.0, 7.0, 10.0, 15.0, 20.0, 30.0]   # ilk taramanin devami (0.25-3.0)
SL_RATIOS_OB = [5.0, 7.0, 10.0, 15.0, 20.0, 30.0, 50.0]    # ilk taramanin devami (0.5-5.0)
...
def _fvg_signals(candles, config, sl_ratio):
    ...
    for f in detect_fvgs(candles, config=config):
        ...
        out.append(Signal(index=f.end_index, type=SignalType.BUY if is_bull else SignalType.SELL,
                          ...
                          breakeven_trigger_pct=be_pct, reason="fvg-slrecal-ext"))
    return out
...
def _ob_signals(candles, config, sl_ratio):
    ...
    for ob in detect_order_blocks(candles, config=config):
        ...
        out.append(Signal(index=ob.impulse_index, type=SignalType.BUY if is_bull else SignalType.SELL,
                          ...
                          entry=entry, stop_loss=stop_loss, take_profit=take_profit,
                          breakeven_trigger_pct=be_pct, reason="ob-slrecal-ext"))
    return out
...
def main():
    ...
    for symbol in KEPT_SYMBOLS:
        ...
        for sl_ratio in SL_RATIOS_FVG:
            ...
            print(f"  fvg sl={sl_ratio:5.2f} n={metrics.filled_trades:5} win={metrics.win_rate:.1%} "
        ...
        for sl_ratio in SL_RATIOS_OB:
            ...
            print(f"  ob  sl={sl_ratio:5.2f} n={metrics.filled_trades:5} win={metrics.win_rate:.1%} "
                  f"exp={metrics.expectancy_r:.4f} pf={metrics.profit_factor:.2f}", flush=True)
        results[symbol] = sym_result

    with open("sl_buffer_recalibration_extended_results.json", "w") as f:
        json.dump(results, f, indent=2, default=str)
    print("\nyazildi: sl_buffer_recalibration_extended_results.json")
    ...
    for mod_name, ratios in [("fvg", SL_RATIOS_FVG), ("ob", SL_RATIOS_OB)]:
        ...
        for sl_ratio in ratios:
            ...
            print(f"  sl={sl_ratio:5.2f} n={n:6} win={wins/n:.1%} exp={total_r/n:.4f}")
