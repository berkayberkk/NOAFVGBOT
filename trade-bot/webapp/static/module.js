...
function paintChart(canvas, ex, view, hoverIdx) {
  ...
  // Fiyat araligi: GORUNEN mumlar + entry/sl/tp (zoom'dan bagimsiz her zaman
  // referans olsun diye) -- TradingView'in "autoscale visible range" davranisi.
  ...
  if (entryBarIdx >= 0 && ex.tp !== null && ex.tp !== undefined && inView(entryBarIdx)) {
  ...
  // Dokunus/Giris isareti
  if (entryBarIdx >= 0 && inView(entryBarIdx)) {
    const ex_ = xOf(entryBarIdx), ey = yOf(ex.entry);
...
function outcomeBadge(outcome) {
  const map = { win: ["Kazandı", "pos"], loss: ["Kaybetti", "neg"], breakeven: ["Breakeven", ""] };
  const [label, cls] = map[outcome] || [outcome, ""];
  return `<span class="outcome-badge outcome-${outcome}">${label}</span>`;
}
